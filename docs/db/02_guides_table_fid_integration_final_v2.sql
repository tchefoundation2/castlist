-- =====================================================
-- Castlist Database Schema: Guides Table with FID Integration (FINAL v2)
-- =====================================================
-- 
-- This script updates the existing guides table to work with FID
-- and creates the missing tables for the complete system.
-- FIXED: Checks for existing columns before creating.
--
-- Run this script AFTER 01_profiles_table_fid.sql
-- =====================================================

-- 1. Add creator_fid column to existing guides table
ALTER TABLE public.guides ADD COLUMN IF NOT EXISTS creator_fid INTEGER;

-- 2. Add cover_image column if it doesn't exist
ALTER TABLE public.guides ADD COLUMN IF NOT EXISTS cover_image TEXT;

-- 3. Create books table (using BIGINT for compatibility)
...
-- 4. Add isbn column to books table if it doesn't exist
...
-- Add user_fid column if it doesn't exist (for existing activities table)
ALTER TABLE public.activities ADD COLUMN IF NOT EXISTS user_fid INTEGER;
ALTER TABLE public.activities ADD COLUMN IF NOT EXISTS activity_type TEXT;
ALTER TABLE public.activities ADD COLUMN IF NOT EXISTS guide_id BIGINT;
ALTER TABLE public.activities ADD COLUMN IF NOT EXISTS book_id BIGINT;
...
-- Add columns if they don't exist (for existing guide_likes table)
ALTER TABLE public.guide_likes ADD COLUMN IF NOT EXISTS user_fid INTEGER;
ALTER TABLE public.guide_likes ADD COLUMN IF NOT EXISTS created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();

-- Add foreign key constraints if they don't exist
DO $$
BEGIN
    -- Add foreign key for activities.user_fid
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'activities_user_fid_fkey' 
        AND table_name = 'activities'
    ) THEN
        ALTER TABLE public.activities 
        ADD CONSTRAINT activities_user_fid_fkey 
        FOREIGN KEY (user_fid) REFERENCES public.profiles(fid) ON DELETE CASCADE;
    END IF;
    
    -- Add foreign key for guide_likes.user_fid
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'guide_likes_user_fid_fkey' 
        AND table_name = 'guide_likes'
    ) THEN
        ALTER TABLE public.guide_likes 
        ADD CONSTRAINT guide_likes_user_fid_fkey 
        FOREIGN KEY (user_fid) REFERENCES public.profiles(fid) ON DELETE CASCADE;
    END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_guides_author_id ON public.guides(author_id); -- Keep existing index
...
-- =====================================================
-- 
-- 1. FINAL v2: Fixed to work with existing books table
-- 2. Adds creator_fid column alongside existing author_id
-- 3. Supports both UUID (author_id) and FID (creator_fid) authentication
-- 4. All new tables use BIGINT for compatibility
-- 5. RLS policies work with both authentication methods
--
-- =====================================================
