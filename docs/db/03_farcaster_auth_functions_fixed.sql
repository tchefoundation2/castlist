-- =====================================================
-- Castlist Database Schema: Farcaster Authentication Functions (FIXED)
-- =====================================================
-- 
-- This script creates helper functions for Farcaster authentication
-- and user session management. FIXED for BIGINT IDs.
--
...
-- 4. Function to get user guides with stats (FIXED for BIGINT)
...
-- 5. Function to get public guides feed (FIXED for BIGINT)
...
-- 6. Function to toggle guide like (FIXED for BIGINT)
CREATE OR REPLACE FUNCTION public.toggle_guide_like(guide_id BIGINT)
RETURNS TABLE (
    liked BOOLEAN,
...
-- 7. Function to create activity log entry (FIXED for BIGINT)
...
-- 8. Function to get recent activities feed (FIXED for BIGINT)
...
GRANT EXECUTE ON FUNCTION public.toggle_guide_like(BIGINT) TO authenticated;
GRANT EXECUTE ON FUNCTION public.log_activity(TEXT, BIGINT, BIGINT, JSONB) TO authenticated;
...
-- =====================================================
-- 
-- 1. FIXED: Changed all UUID references to BIGINT
-- 2. These functions provide a clean API for Farcaster authentication
-- 3. The set_current_user_fid function should be called after verifying
--    the Farcaster signature in your application
-- 4. All functions are security definer to ensure proper RLS context
